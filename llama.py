...
class LayerNorm(torch.nn.Module):
    ...
    def _norm(self, x):
        ...
        # Se obtiene la varianza sin el bias
        var = torch.var(x, dim=-1, unbiased=False, keepdim=True)
        # Fórmula Sugerida para LayerNorm
...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        '''
        # DONE
        # Se aplica la siguiente formula: Attention(Q, K, V) = softmax(Q*K^T/sqrt(d_k))*V
        # Se obtiene la dimensión del key para escalar los scores (d_k)
        d_k = query.size(-1)
        # Se implementa la atención por producto punto escalada 
        scores = torch.matmul(query, key.transpose(-2, -1)) 
        scores = scores / math.sqrt(d_k)
         # Se aplica la función softmax para obtener los pesos de atención
        attn = F.softmax(scores, dim=-1)
        # Se aplica dropout antes de multiplicar por el value
        attn = self.attn_dropout(attn)
        # Se multiplica por el value para obtener la atención de acuerdo a la formula
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        # DONE Para esta parte se guía en la imagen sugerida del Transformer
        # LayerNorm del Input
        x_norm1 = self.attention_norm(x)
        # Attention de la Capa Normalizada
        attn_output = self.attention(x_norm1)
        # Suma de Residual de la entrada
        x_after_attn = x + attn_output
        # LayerNorm del Residual
        x_norm2 = self.ffn_norm(x_after_attn)
        # Paso por la capa feedforward
        ffn_output = self.feed_forward(x_norm2)
        # Suma de Residual de la salida normalizada del Attention
...
class Llama(LlamaPreTrainedModel):
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, epsilon=0.05, top_k=None, top_p=None):
        """
        ...
        * Se añade top_p y top_k sampling para mejorar la calidad de las muestras generadas
        """
        for _ in range(max_new_tokens):
            ...
            if temperature == 0.0:
                ...
                # No se utiliza las probabilidades ya que los logits son proporcionales a las mismas
                idx_next = torch.argmax(logits, dim=-1, keepdim=True)
            else:
                # 1) Se escala los logits dividiendo por la temperatura
                logits = logits / temperature
                # Se convierte los logits a probabilidades con softmax
                probs = F.softmax(logits, dim=-1)
                
                # EXTRA: Se añade top_k y top_p sampling
                ## Se añade top_k 
                if top_k is not None:
                    top_k = min(top_k, probs.size(-1)) # Se elige el menor entre top_k y el tamaño del vocabulario
                    values, _ = torch.topk(probs, top_k) # Se obtienen los top_k valores con torch
                    min_values = values[:, -1, None] # Se obtiene el valor mínimo entre los top_k
                    probs = torch.where(probs < min_values, torch.zeros_like(probs), probs) # Se filtran las probabilidades menores al mínimo entre los top_k
                
                ## Se añade top_p
                if top_p is not None:
                    sorted_probs, sorted_indices = torch.sort(probs, descending=True) # Se ordenan las probabilidades de mayor a menor
                    cumulative_probs = torch.cumsum(sorted_probs, dim=-1) # Se obtiene la suma acumulada de las probabilidades ordenadas

                    mask = cumulative_probs <= top_p # Se crea una máscara para filtrar las probabilidades que cumplen con top_p
                    mask[:, 0] = True # Se asegura que al menos un token sea seleccionado

                    filtered_probs = torch.zeros_like(probs) # Se crea un tensor de ceros para las probabilidades filtradas
                    filtered_probs.scatter_(1, sorted_indices, mask * sorted_probs) # Se asignan las probabilidades filtradas al tensor de ceros
                    probs = filtered_probs

                # 2) Se implementa epsilon sampling (aunque sea redundante con top_k) 
                if epsilon is not None:
                    mask = probs >= epsilon # Se crea una máscara para filtrar los tokens con probabilidad menor a epsilon
                    probs = probs * mask # Se aplica la máscara a las probabilidades

                # 4) Se renormaliza las probabilidades filtradas para que sumen 1
                probs = probs / probs.sum(dim=-1, keepdim=True)
                
                # 5) Se muestrea de la distribución filtrada de probabilidades
