...
class LoRALayer(nn.Module):
    ...
    def forward(self, x):
        ...
        #2 Compute the LoRA path: x -> A -> B -> scale
        #2a: Calcular la Capa A (down-projection)
        lora_A_output = F.linear(x, self.lora_A)
        #2b: Obtener la Capa B a partir de la A (up-projection)
        lora_B_output = F.linear(lora_A_output, self.lora_B)
        #2c: Multiplicar por el factor de escala
        lora_output = lora_B_output * self.scaling
        #3 Añadir la salida de LoRA a la salida original
