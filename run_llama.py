...
def create_data(filename, tokenizer: Tokenizer, flag: str ='train', lower: bool = False, eos: bool = True, prompt_suffix: Optional[str]=None):
	...
	with open(filename, 'r', encoding='utf-8', errors='ignore') as fp: # Se especifica la codificación por un error causado
		for line in fp:
...
def generate_sentence(args, prefix, outfile, max_new_tokens = 75, temperature = 0.0, top_p=None, top_k=None):
	with torch.no_grad():
		device = torch.device('cuda') if args.use_gpu else torch.device('cpu')
		...
		with torch.no_grad():
			with ctx:
				y = llama.generate(x, max_new_tokens, temperature=temperature, top_p=top_p, top_k=top_k)
